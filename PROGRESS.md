...
| Q-7 Languages | Norwegian + English only, never auto-detected: the language is chosen by which hotkey you press and pinned as ElevenLabs `language_code`, which only Flash v2.5 / Turbo v2.5 accept. Kokoro has no Norwegian, so Norwegian reads offline through its own model (an F5-TTS checkpoint, installed separately) and never falls back to an English voice — a language whose offline model is missing is refused, not substituted. |
...
- [x] Norwegian offline voice — the second local engine (`Sources/SRCore/F5/`), an F5-TTS checkpoint (`akhbar/F5_Norwegian`) via `f5-tts-mlx`, sharing Kokoro's venv, daemon and socket. `LocalRuntimeInstaller` owns what the two have in common (venv, hashed lock, daemon script); each installer owns only its model; `LocalVoices` is the one place that maps a language to its engine, so routing, settings, the CLI and the voice picker carry no per-language special cases. The daemon takes an `engine` field (absent = kokoro), loads F5 lazily on the first Norwegian request, and reports a per-engine `output_version` so one engine's audio can never land in the other's cache namespace.
  - **Zero-shot, so a voice is a recording.** F5 has no baked-in speakers: `F5VoiceStore` imports any audio file as a reference clip, converting it to 24 kHz mono with AVFoundation and pairing it with its transcript. The install adopts a sample clip if the repo ships one. The clip's hash is part of the cache variant, so re-recording a voice never replays the old one.
  - **Weaker pinning than Kokoro, deliberately, with a path to the same.** A community fine-tune names no stable revision and follows no file-naming convention, so `daemon/sr_f5_fetch.py` resolves the layout at install time, verifies each download against Hugging Face's own SHA-256, and records the resolved commit plus a hash of every file it writes; launch-time validation re-checks that record (size, not a re-hash of 1.4 GB). `make pin-f5-model` prints the resolved commit as Swift constants to compile in, after which the guarantee matches Kokoro's.
  - **Open decision: the architecture variant.** F5-TTS Base and v1 Base share every tensor shape, so a checkpoint cannot be inspected to tell them apart and the wrong choice produces babble, not an error. sr reads the repo's config when there is one, defaults to Base (what the model card names), and exposes a one-click switch in Settings → Voices. If the upstream card is ever explicit, hard-code it and drop the switch.
